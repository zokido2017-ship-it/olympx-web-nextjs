...
export function AuthShell({ children, className }: AuthShellProps) {
  return (
    <div className="bg-sportxo-page">
      <div className="mx-auto flex w-full max-w-[1440px] flex-col lg:grid lg:min-h-dvh lg:grid-cols-2">
        <AuthBrandPanel />

        <main
          className={cn(
            "flex w-full min-w-0 flex-col items-center px-4 pb-4 pt-5 sm:px-6 sm:pt-6",
            "lg:flex-1 lg:justify-center lg:px-10 lg:py-16",
            className,
          )}
        >
          <div className="w-full min-w-0 max-w-[440px]">{children}</div>
        </main>
      </div>
    </div>
  );
